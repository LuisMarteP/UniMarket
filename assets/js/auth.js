...
btnRegistro.addEventListener('click', async (e) => {
    ...
    if (!usuario.nombre || !usuario.correo || !usuario.password) {
        mostrarError('Por favor, completa todos los campos obligatorios.', document.querySelector('#formRegistro .error-text'));
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarError(data.message || 'Error al registrar.', document.querySelector('#formRegistro .error-text'));
        }
    } catch (error) {
        console.error('Error al enviar el formulario de registro:', error);
        mostrarError('Error en el servidor. Intenta nuevamente más tarde.', document.querySelector('#formRegistro .error-text'));
    }
});
...
btnLogin.addEventListener('click', async (e) => {
    ...
    if (!correo || !password) {
        mostrarError('Por favor, ingresa tus credenciales.', document.querySelector('#formLogin .error-text'));
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarError(data.message || 'Error al iniciar sesión.', document.querySelector('#formLogin .error-text'));
        }
    } catch (error) {
        console.error('Error al enviar el formulario de inicio de sesión:', error);
        mostrarError('Error en el servidor. Intenta nuevamente más tarde.', document.querySelector('#formLogin .error-text'));
    }
});